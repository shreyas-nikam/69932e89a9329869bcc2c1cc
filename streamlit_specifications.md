...
# Streamlit Application Specification: Ensemble Model for Robust Corporate Bond Default Prediction

## 1. Application Overview

**Purpose of the application:**
This Streamlit application provides a practical demonstration for CFA Charterholders and Investment Professionals on building and evaluating robust corporate bond default prediction models using ensemble learning. It emphasizes the integration of diverse data sources (fundamental, market, NLP) to enhance predictive stability and interpretability, addressing regulatory requirements like SR 11-7 concerning challenger models. The app simulates a real-world workflow to help financial professionals understand the benefits of model diversity and ensemble strategies in risk management.

**High-level story flow of the application:**
The application guides the user, acting as a Credit Analyst, through a structured workflow:

1.  **Data Preparation:** The user starts by loading and preparing a simulated credit dataset, understanding how diverse feature sets (fundamental, market, NLP) are defined for different models.
2.  **Base Model Training:** Next, the user trains three distinct base models—Logistic Regression (fundamental), XGBoost (market signals), and LightGBM (NLP features)—each capturing different dimensions of default risk. The diversity of these models is then visualized.
3.  **Ensemble Strategies:** The user proceeds to implement and train three ensemble strategies: Simple Probability Averaging, a Voting Classifier, and Stacking with a meta-learner. The underlying mathematical principles of ensemble diversity are explained.
4.  **Performance & Stability Analysis:** The application then allows the user to evaluate the performance of all individual and ensemble models, focusing on AUC scores, ROC curves, and the crucial aspect of stability demonstrated through bootstrap confidence intervals.
5.  **Model Agreement & Triage:** Finally, the user analyzes the agreement and disagreement among base models, implementing a "triage system" to identify cases requiring human review. This step connects directly to operational risk management and regulatory compliance.
...
from source import * # This imports all functions and variables from source.py
```

### `st.session_state` Initialization, Updates, and Reads:

The application will use `st.session_state` to maintain the state across user interactions and page navigations.

**Initialization (at the start of `app.py`, before any page logic):**
...
if 'agreement_threshold' not in st.session_state:
    st.session_state.agreement_threshold = 0.15 # Default threshold
...
### Application Structure, Flow, and UI Interactions:

The application will use a sidebar `st.selectbox` for navigation, simulating a multi-page experience.
...
if st.session_state.data_loaded:
    ...
    if st.session_state.models_trained:
        ...
        # Invoking `source.py` visualization function
    ...
    if st.button("Train Ensemble Models", help="Trains Averaging, Voting, and Stacking ensembles."):
        with st.spinner("Training ensemble models..."):
            ...
            voting_soft_model, stacking_model, meta_learner_coefs = train_ensemble_strategies(
                ...
                st.session_state.trained_model_fund_pipeline # Pass the *trained* pipeline for consistent scaling
            )
...
if st.session_state.ensembles_trained:
    if st.button("Evaluate All Models", help="Calculates AUCs, Lift, and Bootstrap CIs for all models."):
        with st.spinner("Evaluating models and running stability analysis..."):
            # Update all_auc_scores_dict from previous steps
...
```python
if st.session_state.ensembles_trained:
    # Slider for agreement threshold
